    ...
    <div className="min-h-screen bg-gray-50 pb-32" dir="rtl">
      ...
      <div className="bg-[#1D63ED] pt-10 pb-20 px-6 rounded-b-[3.5rem] shadow-xl relative overflow-hidden text-right">
        <div className="flex justify-between items-center relative z-10 mb-6">
          {/* أيقونة الملف الشخصي الجديدة التي تنقل الطالب لصفحته */}
          <button 
            onClick={() => router.push('/profile')} 
            className="w-12 h-12 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center text-xl border border-white/30 active:scale-90 transition-all shadow-lg"
          >
            👤
          </button>
        ...
        <div className="relative z-10 text-white flex justify-between items-end">
          <div>
            ...
            <div className="flex gap-2 mt-4">
               <button onClick={() => router.push('/leaderboard')} className="bg-[#FFC107] text-[#5D4037] px-4 py-2 rounded-2xl font-black text-[10px] shadow-lg active:scale-95 transition-all">👑 لوحة الشرف</button>
            </div>
          </div>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {lessons.map((lesson, idx) => (
              <div key={idx} className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-gray-100 flex flex-col gap-4 text-right transform transition-all active:scale-[0.99]">
                ...
                <div className="flex gap-2 pt-2 border-t border-gray-50">
                  {lesson.keywords && (
                    <button 
                      ...
                      className="flex-1 bg-indigo-600 text-white py-3 rounded-2xl text-center text-[11px] font-black shadow-md active:scale-95 transition-all"
                    >
                      🧠 تحدي الكلمات
                    </button>
                  )}
      ...
      {/* Navigation Bar شريط التنقل السفلي الموحد */}
      <div className="fixed bottom-6 left-6 right-6 bg-white/90 backdrop-blur-md p-3 rounded-[2.5rem] shadow-2xl border border-gray-100 flex justify-around items-center z-50">
        ...
        <button className="flex flex-col items-center gap-1 p-2">
          <div className="bg-[#1D63ED] text-white w-14 h-14 flex items-center justify-center rounded-full shadow-lg -mt-10 border-4 border-gray-50 transition-all">
        ...
        >
          <span className="text-2xl opacity-60">👤</span>
          <span className="text-[10px] font-black text-gray-400">ملفي</span>
        </button>
      </div>
