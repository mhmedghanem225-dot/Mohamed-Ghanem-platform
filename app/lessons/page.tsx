  ...
  useEffect(() => {
    ...
    const fetchLessons = async () => {
      try {
        ...
        // جلب البيانات مع مراعاة الأعمدة الجديدة G و H
        const filtered = rows.slice(1)
          .map(r => ({ 
            ...
            pdf: r[3]?.trim(), 
            duration: r[4]?.trim(),
            keywords: r[6]?.trim() // العمود G للكلمات
          }))
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24" dir="rtl">
      ...
      <div className="bg-blue-600 pt-10 pb-20 px-6 rounded-b-[3.5rem] shadow-xl relative overflow-hidden text-right">
        <div className="flex justify-between items-center relative z-10 mb-6">
          <button 
            ...
            className="bg-white/20 backdrop-blur-md text-white px-4 py-2 rounded-xl font-bold text-sm transition-all active:scale-95"
          >
            🚪 خروج
          </button>
      ...
      <div className="max-w-md mx-auto px-4 mt-6">
        {loading ? (
          <div className="bg-white p-10 rounded-[2.5rem] shadow-xl text-center font-bold text-blue-600 animate-pulse text-sm">
              ...
              <div key={idx} className="bg-white p-5 rounded-[2rem] shadow-sm border border-gray-100 flex flex-col gap-4 text-right">
                <div className="flex items-center justify-between">
                  ...
                  <a 
                    href={lesson.video} 
                    target="_blank" 
                    className="bg-blue-600 text-white w-12 h-12 rounded-full flex items-center justify-center shadow-lg shadow-blue-100 flex-shrink-0 transition-transform active:scale-90"
                  >
                    ▶️
                  </a>
                </div>

                {/* أزرار الإضافات - تحدي الكلمات والملخص */}
                <div className="flex gap-2 pt-2 border-t border-gray-50">
                  {lesson.keywords && (
                    <button 
                      onClick={() => router.push(`/flashcards?title=${encodeURIComponent(lesson.title)}&grade=${encodeURIComponent(grade)}`)}
                      className="flex-1 bg-orange-500 text-white py-3 rounded-xl text-center text-[10px] font-black shadow-md active:scale-95 transition-all"
                    >
                      🧠 تحدي الكلمات
                    </button>
                  )}
                  {lesson.pdf && (
                    <a 
                      href={lesson.pdf} 
                      target="_blank" 
                      className="flex-1 bg-green-50 text-green-600 py-3 rounded-xl text-center text-[10px] font-black border border-green-100 flex items-center justify-center transition-all active:scale-95"
                    >
                      📄 ملخص الدرس
                    </a>
                  )}
      ...
      <a 
        ...
        target="_blank" 
        className="fixed bottom-6 left-6 bg-[#25D366] text-white w-16 h-16 rounded-full shadow-2xl flex items-center justify-center text-3xl z-50 transition-transform active:scale-90"
      >
      ...
      <footer className="mt-10 text-center text-gray-300 text-[10px] font-bold pb-10">
        Ghanem Academy • 2026
      </footer>
    </div>
  );
