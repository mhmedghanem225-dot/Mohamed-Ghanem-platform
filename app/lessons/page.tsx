...
function LessonsContent() {
  ...
  useEffect(() => {
    ...
    // 2. تحديث النقاط من الذاكرة المحلية فوراً
    const savedPoints = localStorage.getItem("ghanem_points") || "0";
    setPoints(parseInt(savedPoints));

    // 3. جلب الدروس من الشيت
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24" dir="rtl">
      {/* الهيدر الأزرق الملكي الأصلي - كما يظهر في صورتك */}
      <div className="bg-[#1D63ED] pt-10 pb-20 px-6 rounded-b-[3.5rem] shadow-xl relative overflow-hidden text-right">
        ...
        <div className="relative z-10 text-white flex justify-between items-end">
          <div>
            <p className="text-blue-100 font-bold text-sm mb-1 text-right">أهلاً بك يا بطل، {studentName} 👋</p>
            <h1 className="text-3xl font-black mb-1">{grade}</h1>
            <button 
              onClick={() => router.push('/honor-roll')} 
              className="mt-4 bg-[#FFC107] hover:bg-yellow-500 text-[#5D4037] px-5 py-2 rounded-2xl font-black text-[11px] shadow-lg flex items-center gap-2 transition-all active:scale-95"
            >
              👑 لوحة أبطال الأسبوع
            </button>
          </div>
          
          {/* عداد النقاط المضاف للهيدر */}
            ...
            {lessons.map((lesson, idx) => (
              <div key={idx} className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-gray-100 flex flex-col gap-4 text-right">
                ...
                <div className="flex gap-2 pt-2 border-t border-gray-50">
                  ...
                  {lesson.pdf && (
                    <a href={lesson.pdf} target="_blank" className="flex-1 bg-[#E8F5E9] text-[#2E7D32] py-3 rounded-2xl text-center text-[11px] font-black border border-green-100 active:scale-95 transition-all flex items-center justify-center">📄 ملخص الدرس</a>
                  )}
                </div>
              </div>
            ))}
