  ...
  useEffect(() => {
    const fetchLeaders = async () => {
      try {
        ...
        // جلب الأسماء من العمود السادس (index 5) والنقاط من العمود الثامن (index 7)
        const leaderData = rows.slice(1)
          .map(r => ({ 
            name: r[5]?.trim(), 
            points: parseInt(r[7]?.trim() || "0") 
          }))
          .filter(s => s.name && !isNaN(s.points)) // التأكد من وجود اسم ورقم
          ...
          students.map((student, idx) => (
            <div key={idx} className={`bg-white p-4 rounded-2xl flex justify-between items-center shadow-sm border ${idx === 0 ? 'border-yellow-400 ring-2 ring-yellow-100' : 'border-gray-100'}`}>
              <div className="flex items-center gap-3">
                ...
                <span className="font-bold text-gray-800 truncate max-w-[150px]">{student.name}</span>
              </div>
              <span className="font-black text-blue-600 whitespace-nowrap">{student.points} pt</span>
            </div>
          ))
        )}
      </div>
