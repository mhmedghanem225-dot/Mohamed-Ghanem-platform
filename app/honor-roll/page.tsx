  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20" dir="rtl">
      <div className="bg-gradient-to-r from-orange-500 to-yellow-500 pt-10 pb-20 px-6 rounded-b-[3.5rem] shadow-xl relative overflow-hidden text-right">
        <button onClick={() => router.back()} className="bg-white/20 backdrop-blur-md text-white px-4 py-2 rounded-xl font-bold text-sm mb-6 relative z-10">🔙 رجوع</button>
        <div className="relative z-10 text-white text-center">
             <h1 className="text-3xl font-black mb-2">أبطال الأسبوع 👑</h1>
      ...
      <div className="max-w-md mx-auto px-4 mt-8 space-y-6">
        {loading ? (
          <div className="text-center py-10 font-bold text-orange-600 animate-pulse text-sm">جاري تحضير الأبطال...</div>
        ) : stars.map((star, idx) => (
            <div key={idx} className="bg-white p-5 rounded-[2.5rem] shadow-md border border-orange-50 flex items-center gap-5 relative overflow-hidden">
              
              {/* تعديل عرض الصورة لاستخدام tag img العادي لضمان التوافق مع الروابط الخارجية */}
              <div className="w-20 h-20 rounded-full border-4 border-yellow-400 overflow-hidden flex-shrink-0 shadow-lg bg-gray-100">
              ...
              <div className="flex-1 text-right">
                <h3 className="font-black text-gray-800 text-lg">{star.name}</h3>
                <p className="text-blue-600 text-xs font-bold">{star.grade}</p>
                <div className="mt-2 bg-yellow-100 text-yellow-700 px-3 py-1 rounded-full text-[10px] font-black inline-block border border-yellow-200">
                  {star.title}
                </div>
              </div>
            </div>
