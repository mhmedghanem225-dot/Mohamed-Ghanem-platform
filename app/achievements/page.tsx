  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-6 text-right" dir="rtl">
      ...
      <div className="grid grid-cols-1 gap-6 max-w-md mx-auto">
        {/* كارت النقاط - الحفاظ على الهوية البصرية */}
        ...
        <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 text-center">
          ...
          <p className="text-gray-400 text-sm font-bold mb-6 italic">Unlock your English Certificate after 500 points!</p>
          
          {points >= 500 ? (
            <button 
              onClick={() => window.print()} 
              className="w-full bg-[#FFC107] text-[#5D4037] py-4 rounded-2xl font-black shadow-lg animate-bounce active:scale-95 transition-all"
            >
              Print Your Certificate 📄
            </button>
          ) : (
            <div className="w-full bg-gray-100 text-gray-400 py-4 rounded-2xl font-black">
              Locked 🔒 (Need {500 - points} more pts)
            </div>
          )}
        </div>
      </div>

      {/* --- تصميم الشهادة الإنجليزية (يظهر عند الطباعة فقط) --- */}
      <div className="hidden print:block fixed inset-0 bg-white p-12 border-[15px] border-[#1D63ED] text-center font-serif" dir="ltr">
        {/* إطار داخلي للزينة */}
