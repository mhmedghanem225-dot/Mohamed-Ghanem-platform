...
function QuizContent() {
  ...
  const handleAnswer = (selected: string) => {
    ...
    if (currentQ < questions.length - 1) {
      ...
      // نهاية الاختبار - التحقق من المكافأة
      if (finalScore === questions.length) {
        ...
        if (!completed.includes(lessonTitle)) {
          ...
          // 👈 السطر المضاف: تحديث الشيت تلقائياً بالنقاط الجديدة
          updateProgressOnSheet("quiz", 100); 
        }
      }
      setShowResult(true);
    }
      ...
      <div className="min-h-screen bg-[#F0F7FF] flex flex-col items-center justify-center p-6 text-center" dir="rtl">
        <div className="bg-white rounded-[3rem] p-10 shadow-2xl w-full max-w-sm">
          ...
          {isPerfect ? (
            ...
            <p className="text-orange-500 font-black mb-8">يجب حل جميع الأسئلة صحيحة للحصول على النقاط</p>
          )}
